...
# The number of epochs used in the paper
PAPER_EPOCHS: int = 30
# The learning rate used in the original paper
PAPER_LEARNING_RATE: float = 0.002
...
def build_model(
    n_features: int, n_classes: int, metric="accuracy", learning_rate: float = 0.002
):
    ...
    model.add(
        layers.Dense(
            512, kernel_initializer=initializers.glorot_uniform(seed=0), name="fc1"
        )
    )
    model.add(layers.BatchNormalization())
    ...
    model.add(
        layers.Dense(
            512, kernel_initializer=initializers.glorot_uniform(seed=0), name="fc2"
        )
    )
    model.add(layers.BatchNormalization())
