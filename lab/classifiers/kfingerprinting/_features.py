...
def interarrival_stats(list_data: Trace, allow_unidir: bool = False) -> dict:
    ...
    return {
        ...
        **_prefix_keys(_interarrival_stats(overall), ["interarrival", "overall"]),
    }
...
def time_percentiles(overall: Trace, allow_unidir: bool = False) -> dict:
    ...
    def _percentiles(trace):
        ...
        percentiles = (
            np.percentile(times, [25, 50, 75, 100]) if len(times) > 0 else [np.nan] * 4
        )
...
def packet_concentration_stats(overall, chunk_size: int) -> Tuple[dict, Sequence[int]]:
    ...
    if isinstance(overall, np.recarray):
        ...
        is_outgoing_pkt = np.fromiter(
            (pkt.direction == Direction.OUT for pkt in overall),
            dtype=bool,
            count=len(overall),
        )
    ...
    return (
        _prefix_keys(
            {
                "std::out": np.std(concentrations) if concentrations else 0,
                "mean::out": np.mean(concentrations) if concentrations else 0,
                "median::out": np.median(concentrations) if concentrations else 0,
                "min::out": np.min(concentrations) if concentrations else 0,
                "max::out": np.max(concentrations) if concentrations else 0,
            },
            "concentration-stats",
        ),
        concentrations,
...
def alternate_concentration(concentration: Sequence[int], length: int) -> Sequence[int]:
...
def alternate_packets_per_second(
    pps: Sequence[int], length: int
    ...
    # may return more chunks than requested.
    result = np.fromiter(
        (np.sum(group) for group in np.array_split(pps, length)),
        count=length,
        dtype=int,
    )
...
def packets_per_second_stats(overall: Trace) -> Tuple[dict, Sequence[int]]:
    """Return the mean, std, min, median and max number of packets per
    second, as well as the number of packets each second.
    """
    assert len(overall) == 0 or overall[0].timestamp == 0, "trace must starts from zero"
    n_seconds = math.ceil(overall[-1].timestamp) if len(overall) > 0 else 0
    ...
    is_empty = len(packets_per_sec) == 0
    return {
        "pps::mean": np.mean(packets_per_sec) if not is_empty else 0,
        "pps::std": np.std(packets_per_sec) if not is_empty else 0,
        "pps::median": np.median(packets_per_sec) if not is_empty else 0,
        "pps::min": np.min(packets_per_sec) if not is_empty else 0,
        "pps::max": np.max(packets_per_sec) if not is_empty else 0,
    }, packets_per_sec
...
def packet_ordering_stats(overall: Trace) -> dict:
    ...
    if isinstance(overall, np.recarray):
        ...
        in_preceeding = np.asarray(
            [i for i, pkt in enumerate(overall) if pkt.direction == Direction.IN]
        )
        out_preceeding = np.asarray(
            [i for i, pkt in enumerate(overall) if pkt.direction == Direction.OUT]
        )

    return {
        "packet-order::out::mean": np.mean(out_preceeding)
        if len(out_preceeding) > 0
        else np.nan,
        "packet-order::in::mean": np.mean(in_preceeding)
        if len(in_preceeding) > 0
        else np.nan,
...
def in_out_fraction(overall: Trace) -> dict:
    ...
    # Use 1 as the total number of packets if there are no packets to avoid
    # divide by zero below (result is still 0 as expected).
    n_packets = max(counts["packet-counts::overall"], 1)
    return {
...
def total_packet_sizes(overall: Trace, allow_unidir: bool = False) -> dict:
    ...
    result["total-size::overall"] = result["total-size::in"] + result["total-size::out"]
    return result
...
# FEATURE FUNCTION
# ----------------
def make_trace_array(timestamps: Sequence[float], sizes: Sequence[float]) -> np.ndarray:
    """Create a trace-like array from the sequence of timestamps and
    signed sizes.
    """
    assert len(timestamps) == len(sizes)

    trace_array = np.recarray(
        (len(timestamps),),
        dtype=[
...
def _run_extraction(
    idx, directory: str, max_size: int, allow_unidir: bool, allow_empty: bool
):
    ...
    with tempfile.TemporaryDirectory(prefix="kfp-extract-") as directory:
        ...
        with multiprocessing.Pool(n_jobs) as pool:
            ...
            for i, batch in enumerate(
                ...
                _LOGGER.info(
                    "Extraction is %.2f%% complete.", ((i + 1) * 100 / n_chunks)
                )
...
def extract_features_sequence(
    ...
    Setting allow_empty to True forces allow_unidir to also be True.
    """
...
def extract_features(
    ...
    """
    allow_unidir = True if allow_empty else allow_unidir

    if trace is None and (timestamps is None or sizes is None):
        raise ValueError("timestamps and sizes must be specified when trace is None.")
    if trace is not None and (timestamps is not None or sizes is not None):
        raise ValueError(
            "Either trace or both sizes and timestamps should be specified."
        )
    if trace is None:
    ...
    assert len(trace) == 0 or trace[0].timestamp == 0
...
DEFAULT_TIMING_FEATURES = [
    # Interarrival stats
    "interarrival::in::max",
    "interarrival::out::max",
    "interarrival::overall::max",
    "interarrival::in::mean",
    "interarrival::out::mean",
    "interarrival::overall::mean",
    "interarrival::in::std",
    "interarrival::out::std",
    "interarrival::overall::std",
    "interarrival::in::percentile-75",
    "interarrival::out::percentile-75",
    "interarrival::overall::percentile-75",
    # Timestamp percentiles
    "time::in::percentile-25",
    "time::in::percentile-50",
    "time::in::percentile-75",
    "time::in::percentile-100",
    "time::out::percentile-25",
    "time::out::percentile-50",
    "time::out::percentile-75",
    "time::out::percentile-100",
    "time::overall::percentile-25",
    "time::overall::percentile-50",
    "time::overall::percentile-75",
    "time::overall::percentile-100",
    # Packet counts
    "packet-counts::in",
    "packet-counts::out",
    "packet-counts::overall",
    # First and last 30 packet concentrations
    "first-30::packet-counts::in",
    "first-30::packet-counts::out",
    "last-30::packet-counts::in",
    "last-30::packet-counts::out",
    # Some concentration stats
    "concentration-stats::std::out",
    "concentration-stats::mean::out",
...
DEFAULT_SIZE_FEATURES = [
    ...
    "size-stats::out::var",
    "size-stats::out::std",
    "size-stats::overall::mean",
    "size-stats::overall::max",
    "size-stats::overall::var",
    "size-stats::overall::std",
]

ALL_DEFAULT_FEATURES = list(
    itertools.chain(
        DEFAULT_TIMING_FEATURES,
